...
const logoutZoom = async (zoomToken) => {
  const logoutResponse = await fetch(`https://zoom.us/oauth/revoke?token=${zoomToken}&client_id=${process.env.ZOOM_CLIENT_ID}&client_secret=${process.env.ZOOM_CLIENT_SECRET}`, {
    method: 'POST'
  });
  const logoutResponseJson = await logoutResponse.json();
  return logoutResponseJson;
};
